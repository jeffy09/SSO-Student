...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['reset_id_card'])) {
    try {
        ...
        // อัพเดตรหัสบัตรประชาชน
        $update_query = "UPDATE students SET id_card = :id_card, first_login = 1 WHERE id = :id";
        $update_stmt = $db->prepare($update_query);
        ...
        if ($update_stmt->execute()) {
            // บันทึก log
            $log_action = "รีเซ็ตรหัสบัตรประชาชน: " . $student['student_id'];
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <div class="alert alert-info">
                    ...
                    <strong>หมายเหตุ:</strong> การรีเซ็ตรหัสบัตรประชาชนจะทำให้นักศึกษาต้องใช้รหัสใหม่ในการเข้าสู่ระบบครั้งถัดไป
                </div>
                ...
                <form action="?page=admin_edit_user&id=<?php echo $_GET['id']; ?>" method="post" id="resetPasswordForm">
                    <div class="mb-3">
                        ...
                        <div class="form-text">กรุณากรอกรหัสบัตรประชาชน 13 หลัก (ตัวเลขเท่านั้น)</div>
                    </div>
...
document.getElementById('resetPasswordForm').addEventListener('submit', function(e) {
    ...
    Swal.fire({
        ...
        html: `คุณต้องการรีเซ็ตรหัสบัตรประชาชนเป็น<br><strong>${newIdCard}</strong><br>หรือไม่?`,
        icon: 'warning',
        showCancelButton: true,
...
/* เพิ่ม style สำหรับ form container */
.form-container {
    max-width: 1200px;
    margin: 0 auto;
}

/* Style สำหรับ section headers */
.section-header {
...
/* เพิ่ม style สำหรับ required indicator */
...
/* Style สำหรับ card shadows */
.card {
    box-shadow: 0 0.15rem 1.75rem 0 rgba(58, 59, 69, 0.15) !important;
}

.card:hover {
    box-shadow: 0 0.35rem 2rem 0 rgba(58, 59, 69, 0.2) !important;
    transition: all 0.3s ease;
}

/* เพิ่ม style สำหรับ form groups */
.form-group {
    margin-bottom: 1.5rem;
}
...
/* Style สำหรับ input icons */
.input-group-text {
    background-color: #f8f9fa;
    border-color: #ced4da;
}

/* เพิ่ม style สำหรับ toast notifications */
.toast-container {
    position: fixed;
    top: 1rem;
    right: 1rem;
